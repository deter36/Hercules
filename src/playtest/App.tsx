...
export function App() {
  ...
  return <main>
    ...
    <section className="dashboard">
      ...
      <article className="card"><h2>Labor</h2>{view.labor ? <><h3>{view.labor.name}</h3><p>Mood: {view.mood.name ?? "—"}</p>{view.mood.effect && <p className="mood-effect">{view.mood.effect}</p>}{view.labor.dice.map(die => <div className="labor-die" key={die.id}><b>{die.id}</b><span>Health {die.health}/{die.startingHealth}</span><span>{die.trackId} · {die.nodeId}</span><small>{die.status}{die.nodeEffect ? ` · ${JSON.stringify(die.nodeEffect)}` : ""}</small></div>)}</> : <p>Preparing the first Labor…</p>}</article>
