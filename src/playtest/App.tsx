...
export function App() {
  ...
  return <main>
    ...
    <section className="dashboard">
      <article className="card"><h2>Hercules</h2><div className="resources"><span>Spirit <b>{view.player.spirit}</b></span><span>Divinity <b>{view.player.divinity}</b></span></div><div className="dice">{availableDice.map(die => <div className={`die ${die.allocated || die.locked || die.spent ? "used" : ""}`} key={die.id}><b>{die.id}</b><strong>{die.face ?? "—"}</strong><small>{die.allocated ? "attack" : die.locked ? "gold" : die.spent ? "spent" : die.blueUsed ? "blue used" : die.rollable ? "ready" : "unavailable"}</small></div>)}</div><h3>Rewards</h3><p>{view.rewards.length ? view.rewards.map(reward => reward.name).join(" · ") : "None yet"}</p></article>
      <div className="labor-stack"><article className="card"><h2>Labor</h2>{view.labor ? <><h3>{view.labor.name}</h3>{view.labor.dice.map(die => <div className="labor-die" key={die.id}><b>{die.id}</b><span>Health {die.health}/{die.startingHealth}</span><span>{die.trackId} · {die.nodeId}</span><small>{die.status}{die.nodeEffect ? ` · ${JSON.stringify(die.nodeEffect)}` : ""}</small></div>)}</> : <p>Preparing the first Labor…</p>}</article><article className="card mood-card"><h2>Current Mood</h2><h3>{view.mood.name ?? "—"}</h3>{view.mood.effect && <p className="mood-effect">{view.mood.effect}</p>}</article></div>
