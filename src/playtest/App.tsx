...
function LaborTracks({ labor }: { labor: LaborView }) { return <div className={`labor-tracks ${labor.tracks.length > 1 ? "multi" : ""}`}>{labor.tracks.map(track => <section className={`labor-track ${track.type}`} key={track.id}><h4>{track.label}{track.attack && <small>Attack: {track.attack}</small>}</h4><div className="track-grid">{track.nodes.map(node => { const occupants = labor.dice.filter(die => die.trackId === track.id && die.nodeId === node.id && die.status === "active"); return <div className={`track-node ${occupants.length ? "occupied" : ""} ${node.id === track.startId ? "start-node" : ""}`} key={node.id}><b>{node.id}</b><span>{effectMark(node.effect)}</span>{occupants.map(die => <i key={die.id} title={`${die.id}: ${die.health}/${die.startingHealth}`}>{die.id.replace(/^labor\.L\d+\./, "")}</i>)}{node.next.length > 0 && <small>→ {node.next.join(" · ")}</small>}</div>; })}</div></section>)}</div>; }
