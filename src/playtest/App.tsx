...
export function App() {
  ...
  const submit = (command: EngineCommand) => { try { const result = HerculesEngine.submit(state, command); setState(result.state); setMessage(`${result.transitions.at(-1)?.type ?? "Action complete"}.`); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  const start = () => { try { const result = HerculesEngine.createGame({ difficulty, seed }); setState(result.state); setMessage("New game created."); } catch (error) { setMessage(error instanceof Error ? error.message : String(error)); } };
  ...
  return <main>
    ...
    <section className="dashboard">
      ...
      <article className="card controls"><h2>Actions</h2>{view.pendingDecision && <><h3>{view.pendingDecision.prompt}</h3><p>{view.pendingDecision.type}</p></>}{(["round", "blue", "placement", "decision", "utility"] as const).map(group => grouped(group).length ? <div className="action-group" key={group}><h3>{group}</h3>{grouped(group).map(action => <button key={action.id} onClick={() => submit(action.command)}>{action.label}</button>)}</div> : null)}{view.actions.length === 0 && <p>No engine-provided action is available.</p>}</article>
    </section>
