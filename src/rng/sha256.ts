/** A small synchronous SHA-256 implementation so the certified RNG also runs in browsers. */
const K = [
...
export function sha256(bytes: Uint8Array): Uint8Array {
  ...
  for(let offset=0;offset<data.length;offset+=64){
    for(let i=0;i<16;i++)w[i]=((data[offset+i*4]<<24)|(data[offset+i*4+1]<<16)|(data[offset+i*4+2]<<8)|data[offset+i*4+3])>>>0;
  ...
  const output=new Uint8Array(32); for(let i=0;i<8;i++){output[i*4]=h[i]>>>24;output[i*4+1]=h[i]>>>16;output[i*4+2]=h[i]>>>8;output[i*4+3]=h[i];} return output;
}
export function sha256Hex(text:string):string{return [...sha256(new TextEncoder().encode(text))].map(n=>n.toString(16).padStart(2,"0")).join("");}
