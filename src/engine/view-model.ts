...
function blueActions(state: GameState): { actions: PlayAction[]; abilities: PlayAbility[] } {
  ...
  for (const entry of entries.filter(entry => !used.includes(entry.id))) for (const source of eligibleDice(state).filter(die => !die.blueUsed)) {
    ...
    else if (type === "modify_pip" && Array.isArray(entry.definition.delta)) for (const { delta, after } of entry.definition.delta.filter((value): value is number => typeof value === "number").map(delta => ({ delta, after: entry.definition.wrap === true ? ((source.face! - 1 + delta + 6) % 6) + 1 : Math.max(1, Math.min(6, source.face! + delta)) })).sort((left, right) => left.after - right.after)) actions.push({ id: `${entry.id}:${source.id}:${delta}`, label: `${entry.name}: ${source.id} ${source.face} → ${after}`, group: "blue", command: { type: "USE_BLUE_ABILITY", abilityId: entry.id, sourceDieId: source.id, target: delta } });
    else if (type === "map_value") { const mapping = entry.definition.mapping as Record<string, number> | undefined; if (mapping?.[String(source.face)] !== undefined) actions.push({ id: `${entry.id}:${source.id}`, label: `${entry.name}: use ${source.id}`, group: "blue", command: { type: "USE_BLUE_ABILITY", abilityId: entry.id, sourceDieId: source.id } }); }
