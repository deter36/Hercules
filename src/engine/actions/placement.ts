...
export function useBlueAbility(state: GameState, abilityId: string, sourceDieId: string, target?: number | string): GameState {
  ...
  // linked nonphysical contribution so the original and copy can be assigned
  // independently, without ever duplicating a physical Hercules die.
...
export function placeGoldAbility(state: GameState, abilityId: string, dieIds: string[], contributionIds: string[] = []): GameState {
  ...
  if (new Set(dieIds).size !== dieIds.length || new Set(contributionIds).size !== contributionIds.length) throw new Error("Placement IDs must be unique.");
  const values = [
    ...dieIds.map((id) => { const die = state.herculesDice[id]; if (!die || !canPlace(die) || die.face === null) throw new Error(`Die ${id} is not eligible for gold placement.`); return die.face; }),
    ...contributionIds.map((id) => { const contribution = state.round.derivedContributions[id]; if (!contribution || contribution.allocated) throw new Error(`Contribution ${id} is not eligible for gold placement.`); return contribution.face; })
  ];
  ...
  next.round.goldPlacements.push({ abilityId, dieIds: [...dieIds], contributionIds: [...contributionIds] });
...
export function allocateAttack(state: GameState, targetId: string, dieIds: string[], contributionIds: string[] = []): GameState {
  ...
  const physicalValues = dieIds.map((id) => { const die = state.herculesDice[id]; if (!die || !canPlace(die) || die.face === null) throw new Error(`Die ${id} is not eligible for attack allocation.`); return die.face; });
