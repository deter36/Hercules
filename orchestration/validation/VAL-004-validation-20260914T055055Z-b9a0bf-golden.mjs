...
import { advanceLaborDice, resolveEnteredImpacts } from '../../dist/src/engine/round/progress.js';
...
// Independent observational driver adapted from the prior validation recorder.
// Damage and transition helpers have full state boundaries; the promoted impact
// scheduler executes intact so this validator cannot repair terminal behavior.
const RUN = 'validation-20260914T055055Z-b9a0bf';
const TESTED = '18440b2637c94807213483a73664d8a2a0f3f366';
const ROOT = 'orchestration/validation/VAL-004-' + RUN + '-golden';
...
const task = {...read('orchestration/tasks/VAL-004.json'),validation_mode:'deterministic_replay',scenario_id:'HERC-GOLDEN-RUN-0001',start_state:{complete_state_path:'orchestration/validation/RS-001-golden-start-state.json'},rng:{seed:'HERC-GOLDEN-RUN-0001',algorithm:'SHA256_COUNTER_V1',policy:'HERC-RNG-v3'}};
const manifest = read('orchestration/project_state.json');
ensure(manifest.manifest_revision === 'e1-m0007', 'MANIFEST_CHANGED');
if (!CHECK) {
...
ensure(read('orchestration/validation/VAL-004-validation-20260914T055055Z-b9a0bf-residuals.json').pass_fail==='PASS','PRIOR_RESIDUAL_STOP');
...
function damageAndImpacts() {
  ...
  // Execute the promoted impact scheduler intact, including its chained effects and terminal handling.
  step('Resolve entered impacts with promoted scheduler',resolveEnteredImpacts);
...
try {
  ...
  const nodes=id=>{const m=id.match(/^L(\d+)_([A-Z]\d*|L|R|D1)_N(\d+)$/);ensure(m,'TERMINAL_NODE_MAPPING',{id});return 'L'+m[1].padStart(2,'0')+m[2]+'.n'+m[3];};
  const check=(name,observed,target)=>{terminalChecks.push({name,expected:target,observed,matched:equal(observed,target)});ensure(equal(observed,target),'TERMINAL_STATE_DIVERGENCE',{name,expected:target,observed});};
  check('phase',state.game.phase,expected.phase);check('result',state.game.result,expected.result);
  check('labor',Number(state.game.currentLaborId.slice(-2)),expected.labor);
  check('spirit',state.player.spirit,expected.spirit);check('divinity',String(state.player.divinity)+'/10',expected.divinity);
  check('next_event',Number(state.rng.nextEvent),expected.next_event);check('complete_ledger',state.rng.ledger.length,record.rng_event_ledger.length);
  check('completed_labors',state.game.completedLaborIds.map(id=>Number(id.slice(-2))),expected.completed_labors);
  check('active_mood',GAME_DATA.moods.find(m=>m.id===state.mood.activeMoodId)?.name,expected.active_mood.name);
  check('hidden_mood_order',names(state.mood.deck),expected.ordered_hidden_mood_deck_top_to_bottom);
  check('rewards',state.player.ownedRewardIds,expected.owned_rewards.map(rewardId));check('removed_components',state.player.removedRewardOrComponentIds,expected.removed_components.map(rewardId));
  check('cannot_block',state.currentLabor.cannotBlockThisRound,expected.round_start_restrictions.cannot_block_this_round);
  for(const [external,e] of Object.entries(expected.birds_labor_dice)){const d=state.currentLabor.laborDice[laborDieId(external)];check(external,{health:d.health,status:d.status,node_id:d.nodeId},{health:e.health,status:e.status,node_id:nodes(e.node_id)});}
  const failed=state.currentLabor.laborDice[laborDieId(expected.failure_cause.labor_die_id)];check('failed_die_identity',{id:failed.id,node:failed.nodeId,status:failed.status},{id:'labor.L06.C17',node:nodes(expected.failure_cause.node_id),status:'active_failure_terminal'});
  for(const [id,e] of Object.entries(expected.hercules_dice)){
    const d=state.herculesDice[id];const placement=e.placement===null?null:e.placement.startsWith('attack:')?{kind:'attack',targetId:laborDieId(e.placement.slice(7))}:{kind:'gold',abilityId:abilityId(e.placement+':GOLD')};
    check(id+'_terminal_fields',{face:d.face,blue_used:d.blueUsed,placement:d.placement,locked:d.locked,allocated:d.allocated,broken:d.broken,spent:d.spent,available:d.availableForLabor},{face:e.face,blue_used:e.blue_used!==null,placement,locked:e.locked,allocated:e.allocated,broken:e.broken,spent:false,available:true});
    if(e.history)check(id+'_Cows_reroll_history',d.history.some(h=>h.type==='reroll'&&h.purpose==='labor6:cows_b:reroll2:H5'),true);
  }
  check('unavailable_H8_H11',Object.values(state.herculesDice).filter(d=>Number(d.id.slice(1))>=8).every(d=>!d.availableForLabor&&d.face===null&&!d.rollable&&!d.spent&&!d.broken&&!d.blueUsed&&!d.locked&&!d.allocated&&d.placement===null),true);
  check('no_pending_decision',state.pendingDecision,null);check('no_pending_triggers',state.pendingTriggers,[]);
  check('five_checkpoints',checkpoints.length,5);check('all_112_inputs',consumed.length,112);
  check('no_cleanup_or_reward_after_terminal',trace.at(-1).label,'Resolve entered impacts with promoted scheduler');
  const prior=read('orchestration/validation/VAL-003-validation-20260914T015459Z-5daa2c-golden-checkpoints.json');
  check('VAL003_full_terminal_state_identical',equal(state,prior.terminal_state),true);
  check('VAL003_all_full_checkpoint_states_identical',equal(checkpoints.map(c=>c.full_state),prior.completed_labor_checkpoints.map(c=>c.full_state)),true);
  check('VAL003_command_consumption_identical',equal(consumed,prior.player_choices),true);
  check('VAL003_full_operation_trace_identical',fs.readFileSync('orchestration/validation/VAL-003-validation-20260914T015459Z-5daa2c-golden-trace.jsonl','utf8')===trace.map(t=>JSON.stringify(t)).join('\n')+'\n',true);
