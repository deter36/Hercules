...
const RUN='validation-20260914T015459Z-5daa2c', COMMIT='3172948a749eac19c17551b4e39624638f8835b6';
const STEM='orchestration/validation/VAL-003-'+RUN+'-l08', CHECK=process.argv.includes('--check');
...
const manifest=read('orchestration/project_state.json'),task={...read('orchestration/tasks/VAL-003.json'),scenario_id:'HERC-ORCH-E1-L08'};
requireThat(manifest.manifest_revision==='e1-m0005'&&task.manifest_revision===manifest.manifest_revision,'MANIFEST_CHANGED');
if(!CHECK){
...
if(CHECK){requireThat(equal(read(STEM+'-checkpoints.json'),evidence),'REPRODUCTION_STATE_MISMATCH');requireThat(equal(read(STEM+'-rng-ledger.json'),ledger),'REPRODUCTION_LEDGER_MISMATCH');console.log(JSON.stringify({...summary,reproduction:'EXACT_MATCH; full states/actions/operations and zero-RNG ledger; no files written'},null,2));}
 ...
 fs.writeFileSync(STEM+'-execution-log.txt',[
  `VAL-003 L08 ${RUN}; tested ${COMMIT}; manifest e1-m0005.`,
  'Clean independent checkout, 40 pins/source commits/Git-byte hashes/tree membership verified (three superseded trees are historical only).',
