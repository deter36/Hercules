...
const RUN='validation-20260913T172338Z-cc12bf', COMMIT='20ef2654433cd7dc0b8c77878679f264b1891e34';
const STEM='orchestration/validation/VAL-002-'+RUN, CHECK=process.argv.includes('--check');
...
const manifest=read('orchestration/project_state.json'),task=read('orchestration/tasks/VAL-002.json');
requireThat(manifest.manifest_revision==='e1-m0004'&&task.manifest_revision===manifest.manifest_revision,'MANIFEST_CHANGED');
if(!CHECK){
...
const pins=[...task.input_artifact_ids,'rs-002-handoff'].map(id=>{
 const pin=manifest.canonical_artifacts.find(a=>a.id===id);requireThat(pin,'PIN_MISSING',id);
 const actual=txt('rev-parse',COMMIT+':'+pin.path), source=pin.source_commit?txt('rev-parse',pin.source_commit+':'+pin.path):null;
 const working=pin.kind==='blob'?txt('hash-object','--',pin.path):actual;
 const hash=pin.kind==='blob'?sha(git('cat-file','blob',actual)):null;
 const changes=txt('status','--porcelain=v1','--untracked-files=all','--ignored','--',pin.path);
 const members=pin.kind==='tree'?txt('ls-tree','-r',COMMIT,'--',pin.path).split('\n').map(line=>{const [meta,path]=line.split('\t');return {path,git_object:meta.split(' ')[2],working_object:txt('hash-object','--',path)};}):null;
 return {...pin,observed_commit:COMMIT,observed_git_object:actual,observed_source_object:source,observed_working_object:working,observed_sha256_git_bytes:hash,covered_tree_members:members,working_changes:changes,matched:actual===pin.git_object&&working===actual&&(!source||source===actual)&&(!pin.sha256_git_bytes||hash===pin.sha256_git_bytes)&&changes===''&&(!members||members.every(m=>m.git_object===m.working_object))};
});
requireThat(pins.length===31&&pins.every(p=>p.matched),'INPUT_PIN_MISMATCH',pins.filter(p=>!p.matched));
...
if(CHECK){requireThat(equal(read(STEM+'-checkpoints.json'),evidence),'REPRODUCTION_STATE_MISMATCH');requireThat(equal(read(STEM+'-rng-ledger.json'),ledger),'REPRODUCTION_LEDGER_MISMATCH');console.log(JSON.stringify({...summary,reproduction:'EXACT_MATCH; full states/actions/operations and zero-RNG ledger; no files written'},null,2));}
 ...
 fs.writeFileSync(STEM+'-execution-log.txt',[
  `VAL-002 ${RUN}; tested ${COMMIT}; manifest e1-m0004.`,
  'Clean independent checkout, 31 pins/source commits/Git-byte hashes/tree membership verified.',
